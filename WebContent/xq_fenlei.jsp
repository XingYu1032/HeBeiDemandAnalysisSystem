...
function check_xingshi(){
	var jieguo=document.getElementsByName("jieguo");
	if(jieguo[0].checked == true){
...
<div class="layui-layout layui-layout-admin">
  ...
  <div class="layui-side layui-bg-black">
    <div class="layui-side-scroll">
      ...
      ResultSet rs5=cons.prepareStatement("select * from users where username ='"+names+"'").executeQuery();
      rs5.next();
      String role=rs5.getString("role");
  ...
  <div class="layui-body">
    ...
    <div style="padding: 15px;">
		...
		String url = "select* from xuqiu where Inamenum=" + "'" + Inamenum+"'";
...
//		System.out.println("saveedit界面的Iintro："+Iintro);
		...
		String glbm=rs.getString("Ixqbelong");
		String hzms=rs.getString("Ihzms");
		String type=rs.getString("Itype");
		String fir=rs.getString("fir");
		String sec=rs.getString("sec");
		String thr=rs.getString("thi");
		String ly=rs.getString("Ilingyu");
		
		rs.close();
	%>
	<form id="form2" method="post" action="xqfenlei">
			...
			<tr>
				<td align="center"><table width="900" border="1" align="center">
						...
						<tr>
							...
							<td colspan="3" align="left">  <%=Iname%><%-- <input name="Iname" type="text" readonly=true id="机构名称" size="50" value=<%=Iname%> /> --%></td>
							<td width="140" align="center">归口管理部门</td>
							<td colspan="2" width="168" align="left"> <%=glbm%><!-- <select id="归口管理部门"
								name="Ixqbelong"> -->
			...
			<%-- 						<%
						...
						<tr>
							...
							<td colspan="3" align="left"><%-- <input name="Iadd" type="text"
								id="通讯地址" size="50" value=<%=Iadd%> /> --%> <%=Iadd%></td>
							<td width="140" align="center">所在地域</td>
							<td colspan="2" align="left"><%-- <input name="Iszdy" type="text"
								id="所在地域" size="32" value=<%=Iszdy%> /> --%> <%=Iszdy%></td>
						</tr>
						<tr>
							<td align="center">网址</td>
							<td colspan="3" align="left"> <%=Iweb%><%-- <input name="Iweb" type="text"
								id="网址" size="50" value=<%=Iweb%> /> --%></td>
							<td width="140" align="center">电子信箱</td>
							<td colspan="2" align="left">
							<%-- <input type="text"
								name="Iemail" id="电子信箱" size="32" value=<%=Iemail%> /> --%><%=Iemail%></td>
						</tr>
						<tr>
							<td align="center">法人代表</td>
							<td colspan="3" align="left"><%-- <input name="Ifrdb" type="text"
								id="法人代表" size="15" value=<%=Ifrdb%> /> --%><%=Ifrdb%></td>
							<td align="center">邮政编码</td>
							<td colspan="2" align="left"><%-- <input name="Iyzbm" type="text"
								id="邮政编码" size="32" maxlength="6" value=<%=Iyzbm%> /> --%> <%=Iyzbm%></td>
						</tr>
						<tr>
							<td height="101" rowspan="2" align="center">联系人</td>
							<td height="101" colspan="2" rowspan="2" align="left"><%-- <input
								name="Iclient" type="text" id="联系人" size="15" value=<%=Iclient%> /> --%><%=Iclient%></td>
							<td width="155" align="center">电话</td>
							<td colspan="3" align="left" nowrap="nowrap"><p>
									1：固话 <%-- <input type="text" name="Igh" id="固话" value=<%=Igh%> /> --%><%=Igh%>
								</p>
								<p>
									2：手机<%--  <input type="text" name="Iphone" id="手机" value=<%=Iphone%> /> --%><%=Iphone%>
								</p></td>
						</tr>
						<tr>
							<td align="center">传真</td>
							<td colspan="3" align="left" nowrap="nowrap"><%=Ifax%><%-- <input
								name="Ifax" type="text" id="传真" size="35" value=<%=Ifax%>> --%></td>
						</tr>
						<tr>
							<td align="center">机构属性</td>
							<td colspan="6" align="left"><p>
<!-- 									<input type="radio" name="Ipeoprety" id="机构" value="企业" /> 
									<label for="机构">企业</label> 
									<input type="radio" name="Ipeoprety" id="机构2" value="高等院校" /> 
									<label for="机构2">高等院校</label> 
									<input type="radio" name="Ipeoprety" id="机构3" value="研究机构" /> 
									<label for="机构3">研究机构</label> 
									<input type="radio" name="Ipeoprety" id="机构4" value="其他" /> 
									<label for="机构4">其他</label> -->
									<%=Ipeoprety %>
								</p></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="3" align="left"><%-- <input name="Ixqname"
								type="text" id="技术需求名称" size="50" value=<%=Ixqname%> /> --%><%=Ixqname%></td>
							<td align="center">*需求时限</td>
							<td colspan="2" align="center"><p>
									<%-- <input name="Iyear1" type="text" id="Iyear1" size="8"
										value=<%=Iyear1%> /> --%> <%=Iyear1%>&nbsp;年至 <%-- <input name="Iyear2" type="text"
										id="Iyear2" size="8" value=<%=Iyear2%> /> --%> &nbsp;<%=Iyear2%>年
								</p></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<td colspan="6" align="center"><input readonly="readonly" name="Igjz1"
								type="text" id="关键字1" size="20" value=<%=Igjz1%> /> <input
								name="Igjz2" readonly="readonly" type="text" id="关键字2" size="15" value=<%=Igjz2%> />
								<input name="Igjz3" type="text" readonly="readonly" id="关键字3" size="15"
								value=<%=Igjz3%> /> <input name="Igjz4" readonly="readonly" type="text" id="关键字4"
								size="15" value=<%=Igjz4%> /> <input name="Igjz5" readonly="readonly" type="text"
								id="关键字5" size="15" value=<%=Igjz5%> /></td>
						</tr>
						<tr>
							<td align="center"><p>拟投入资金总额</p></td>
							<td colspan="5" align="left"><%-- <input name="Izjze" type="text"
								id="Izjze" size="10" value=<%=Izjze%> /> --%> <%=Izjze%>&nbsp; 万元</td>
						</tr>
						<tr>
							...
							<td colspan="6" align="left"><!-- <input type="radio"
								...
								for="合作4">其他</label> --><%=hzms %></td>
						</tr>
						<tr>
							<td align="center">合作意向单位</td>
							<td colspan="5" align="left"><%-- <input name="Icompany"
								type="text" id="意向单位" size="108" value=<%=Icompany%>> --%><%=Icompany%></td>
						</tr>
						<tr>
						...
						<table border="1" name="table2" id="table2">
							...
							<table border="1" width="900" name="table3" id="table3">
							<tr id="所属领域">
								<td width="110" align="center"><p id="领域">需求技术所属领域(非基础研究填写)</p></td>
								<td colspan="5" align="center" id="lingyu">
							...
							<tr id="应用行业">
								...
								<p id="行业">需求技术应用行业(非基础研究填写)</p></td>
								<td colspan="5" align="left">   <%=Ixqyyhy%>
								<%-- <input name="Ixqyyhy" 
		...
		<br>
           ...
           <input type="hidden" name="shenhe" value=<%=names%>>
		
				
				<!-- <input type="radio" name="jieguo" id="jieguo" value="形式审核未通过" onclick="check_xingshi()"  style="width:100px;height:20px"  checked="checked"/> <label for="形式审核未通过">形式审核未通过</label>
				 <input type="radio" name="jieguo" id="jieguo" value="形式审核通过" onclick="check_xingshi()" style="width:100px;height:20px"/> <label for="形式审核通过">形式审核通过</label> -->
				 
				 <br>
				 <br>
				 <br>
				 <br>
				 <!-- <textarea rows="10" cols="50" placeholder="请填写形式不通过的原因" name="reason" id="reason" ></textarea> -->
				<label>部门分类&nbsp;&nbsp;&nbsp;&nbsp;</label>
				 <select id="bumen" name="bumen">
				 	...
				 	<option value="20">科技研发中心</option>
				 	<option value="21">科技情报研究院</option>
				 	<option value="22">器材公司</option>
				 	<option value="23">基金办</option>
				 	<option value="24">档案馆</option>
				 	<option value="25">科技管理信息中心</option>
				 	<option value="26">科技投资中心</option>
				 	<option value="27">成果转换中心</option>
				 	<option value="28">中小企业创新资金管理中心</option>
				 	<option value="29">对外交流中心</option>				 
				 
				 </select>
				<br>
				<br>
				<br>
				<input type="submit" name="submit" id="submit" value="提交" style="width:150px;height:30px"> 
